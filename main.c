...
lval *builtin_def(lenv *e, lval *a) {
  ...
  LASSERT(a, syms->count == a->count - 1,
          ...
          "number of values to symbols. Got %i, Expected %i.",
...
lval *lval_eval_sexpr(lenv *e, lval *v) {
  ...
  if (f->type != LVAL_FUN) {
    ...
    lval_del(v);
    lval_del(f);
    return lval_err("First element is not a function. Got %s.", ltype_name(tp));
  }
...
int main(int argc, char **argv) {
  ...
  mpca_lang(MPCA_LANG_DEFAULT, "                        \
    ...
    mlisp  : /^/ <expr>* /$/ ;                          \
  ",
            Number, Symbol, Sexpr, Qexpr, Expr, Mlisp);
  ...
  mpc_cleanup(6, Number, Symbol, Sexpr, Qexpr, Expr, Mlisp);
